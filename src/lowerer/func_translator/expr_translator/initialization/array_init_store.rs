...
use crate::{
    ast_nodes::{Expr, ExprKind, namespace_registry::ArrayID},
        ...
        match &expr.kind {
            ...
            ExprKind::ArraySpread { value, count } => {
                if self.is_zero(value) {
                    ...
                    let offset_ptr = self.builder.ptr_add(dst_ptr, Offset::Immediate(dst_offset));
                    self.builder.memset(item_size * count, 0, offset_ptr);
                } else {
                    for i in 0..*count {
                        let offset = dst_offset + item_size * i;
