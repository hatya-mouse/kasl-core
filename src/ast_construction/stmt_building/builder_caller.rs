...
pub fn build_func_bodies(
    ...
    let func_ids = program.funcs.keys().cloned().collect::<Vec<_>>();
    ...
    for func_id in func_ids {
        ...
        if let Some(func_path) = program.get_path_by_id(&func_id).cloned()
            && let Some(table_stmt_id) = symbol_table
                .get_id_by_path(&func_path)
                .and_then(|ids| ids.first())
        {
            // Get the function body statements from the SymbolTable
            if let Some(func_body_stmts) = match symbol_table.get_statement_by_id(table_stmt_id) {
