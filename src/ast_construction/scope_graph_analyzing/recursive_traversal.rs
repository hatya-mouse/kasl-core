...
impl ScopeGraphAnalyzer<'_> {
    pub fn analyze_scope(
        ...
        let current_scope_size = self.calculate_scope_layout(current_scope, current_offset_base);
        let child_offset_base = current_offset_base + current_scope_size;
        ...
        if let Some(child_scopes) = self.scope_graph.get_callees(current_scope) {
            for child_scope in child_scopes {
                if states.get(child_scope) == Some(&ScopeState::Visiting) {
                    ...
                    self.analyze_scope(child_scope, states, total_sizes, child_offset_base);
    ...
    pub fn calculate_scope_layout(
        &mut self,
        scope_id: &ScopeID,
        current_offset_base: usize,
    ) -> usize {
        let mut offset_pointer = current_offset_base;
        let mut size = 0;
        if let Some(scope) = self.scope_registry.get_scope(scope_id) {
            for var_id in &scope.variables {
                ...
                // Register the offset of the variable
                self.memory_layout.register_offset(*var_id, offset_pointer);
                offset_pointer += var_size;
